...
class click_alarmclock(Base_Task):
    ...
    def load_actors(self):
        ...
        self.add_contact_marker_to_alarm()  
        self.scene.step()    
        self.scene.update_render()            
  
    def add_contact_marker_to_alarm(self):    
        """Attach a bright colored sphere to the alarm's contact point"""    
        if not hasattr(self, 'alarm'):    
            return    
          
        # Get contact point position relative to alarm    
        contact_point_local = self.alarm.get_contact_point(0, "pose").p  
        ...
        # Build and attach to alarm    
        marker = builder.build(name="contact_marker")    
        
        # Set marker pose relative to alarm    
    ...
    def _update_render(self):    
        ...
        # Update marker to follow alarm    
        if hasattr(self, 'contact_marker') and hasattr(self, 'alarm'):    
            contact_point_local = self.alarm.get_contact_point(0, "pose").p  
            marker_pose = sapien.Pose(p=contact_point_local)    
